...
import component_contribution_legacy


#NOTE: This class cannot be used for transport that would involve different pH, ionic strength, etc... values depending on the compartment:
class rpComponentContribution:
    ...
    WARNING: this uses a legacy version of component contribution. For reactions that have unkown chemical species, need to use the latest component contribution
    method.
    """
    def __init__(self,
                 rpsbml=None,
        ...
        self.max_pH = 14.0
        #TODO: test
        self.groups_data = component_contribution_legacy.inchi2gv.init_groups_data()
        self.group_names = self.groups_data.GetGroupNames()
        self.decomposer = component_contribution_legacy.inchi2gv.InChIDecomposer(self.groups_data)
    ...
    def _select_dG(self, cid):
        if cid in self.kegg_dG:
            ...
            try:
                ...
                for cmp_d in sorted(self.kegg_dG[cid]['component_contribution_legacy'], key=lambda k: int(k['CID'][1:])):
                ...
                pmap_species = None
                try:
                    compound_index = self.kegg_dG[cid]['component_contribution_legacy'][0]['compound_index']
                except KeyError:
                    pass
                try:
                    group_vector = self.kegg_dG[cid]['component_contribution_legacy'][0]['group_vector']
                except KeyError:
                    pass
                try:
                    pmap_species = self.kegg_dG[cid]['component_contribution_legacy'][0]['pmap']['species']
                except KeyError:
                ...
                try:
                    if len(self.kegg_dG[cid]['alberty'])>=1:
                        ...
                        self.logger.warning('Alberty and component_contribution_legacy species are empty')
                        raise KeyError
                except KeyError:
    ...
    def scrt_dfG_prime_o(self, srct_type, srct_string, stoichio):
        ...
        #compute pKas
        try:
            p_kas, major_ms_smiles = component_contribution_legacy.chemaxon.get_dissociation_constants(inchi)
        except component_contribution_legacy.exceptions.ChemAxonRuntimeError:
        ...
        atom_bag, major_ms_charge = component_contribution_legacy.compound.atom_bag_and_charge(molecule)
        ...
        except component_contribution_legacy.inchi2gv.GroupDecompositionError as gde:
        ...
        ############################### calculate dG0_r
        G[:len(self.group_names), 0] = g #WARNING: inspired from component_contribution_legacy, here using equilibrator data
    ...
    def species(self, libsbml_species, stoichio, physioParameter=1e-3):
        ...
        miriam_annot = self.rpsbml.readMIRIAMAnnotation(libsbml_species.getAnnotation())
        try:
        ...
        for c in cid:
            try:
                dfG_prime_o, X, G, physioParameter = self.cmp_dfG_prime_o(c, stoichio, physioParameter)
        ...
        if smiles:
            try:
                ...
                pass
        self.logger.warning('Cannot calculate thermodynamics '+str(libsbml_species.getId()))
    ...
    def reaction(self, libsbml_reaction, write_results=False):
        """
        libsbml_reaction: libSBML libsbml_reaction object
        """
        already_calculated = {}
        dfG_prime_o = 0.0
        ...
        for pro in libsbml_reaction.getListOfProducts():
            ...
            if not dfG_prime_o==None:
                ...
                self.logger.warning('There is no dfG_prime_o value for product: '+str(pro))
        for rea in libsbml_reaction.getListOfReactants():
            dfG_prime_o = None
            try:
                if not rea.species in already_calculated:
                    self.logger.debug('======== '+str(pro.species)+' ========')
                    dfG_prime_o, X, G, concentration = self.species(self.rpsbml.model.getSpecies(rea.species), -float(rea.stoichiometry))
                ...
                self.logger.error('Failed to calculate the thermodynamics for '+str(pro.species))
                #TODO: consider returning False if it fails
                continue
            reaction_stoichio.append(-float(rea.stoichiometry))
            reaction_concentration.append(float(concentration))
            if not dfG_prime_o==None:
                dfG_prime_o += dfG_prime_o*(-rea.stoichiometry)
                X_reaction += X
                G_reaction += G
            else:
        ...
        for member in rp_pathway.getListOfMembers():
            ...
            for pro in reaction.getListOfProducts():
                ...
                try:
                    if not pro.species in already_calculated:
                        self.logger.debug('======== '+str(pro.species)+' ========')
                        dfG_prime_o, X, G, concentration = self.species(
                                rpsbml,
    ...
    def isBalanced():
        ...
        """
        #TODO: check the difference between equilibrator and component_contribution_legacy
